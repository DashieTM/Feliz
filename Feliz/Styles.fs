...
type style =
    ...
    static member inline grid (areaName: string, length: ICssUnit) = Interop.mkStyle "grid" (areaName + unbox<string> length)
    /// TODO beforepr comment
    static member inline grid (namesAndLengths: (string * ICssUnit) seq) = Interop.mkStyle "grid" (namesAndLengths |> Seq.map (fun (areaName, length) -> areaName + unbox<string> length) |> String.concat " ")
    /// TODO beforepr comment
    //static member inline grid (namesAndLengths: (string * ICssUnit) seq) = Interop.mkStyle "grid" (namesAndLengths |> Seq.map (fun (areaName, length) -> areaName + unbox<string> length))
    /// Sets the width of each individual grid column in pixels.
    ///
    /// **CSS**
    /// ```css
