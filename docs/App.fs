...
let update msg state =
    ...
    | UrlChanged segments ->
        ...
        | [] when segments.Length > 2 -> segments |> TabToggled |> Cmd.ofMsg
        | _ -> Cmd.none
    | TabToggled tabs ->
        match tabs with
...
// ok
[<ReactComponent>]
let MyComponent (label: string, value: string, onChange: string -> Unit) = Html.textf "Value: %s" value
// ok
[<ReactComponent>]
...
[<ReactComponent>]
let MyComponent2 (value: string, onChange: string -> Unit) = MyComponent ("Name", value, onChange)
...
let addedDisbaledValuesInDropdown =
    React.functionComponent (fun () ->
        let (selectedValue, setSelectedValue) = React.useState<string option> (None)
...
let customizedSearchInDropdown =
    React.functionComponent (fun () ->
        let (selectedValue, setSelectedValue) = React.useState<string option> (None)
        ...
        Html.div [
            ...
            prop.children [
                SelectSearch.selectSearch [
                    ...
                    selectSearch.search true
                    selectSearch.onChange (fun value -> setSelectedValue (Some value))
                    selectSearch.filterOptions (fun item searchQuery ->
                        not item.disabled && item.name.StartsWith searchQuery
                    )
                    selectSearch.options [
                        {
                            value = "en-GB"
...
let selectMultipleValuesFromDropDown =
    React.functionComponent (fun () ->
...
let customizedButtonsInDropdown =
    React.functionComponent (fun () ->
        let imageUrlByValue =
            function
            | "sushi" ->
            ...
            | "ramen" ->
                "https://www.24kitchen.nl/files/styles/social_media_share/public/2019-02/24K_Job%26Perry_HIGHRes_Snelle_Japanse_Ramen_met_misogehakt_en_een_eitje_L_NOLOGO-1.jpg?itok=YD85hSCT"
            | "pasta" ->
                "https://www.24kitchen.nl/files/styles/social_media_share/public/2018-09/Screen%20Shot%202018-09-11%20at%2016.46.22.png?itok=5uoIHWGg"
            | _ ->
...
let samples = [
    ...
    "effectful-tab-counter",
    DelayedComponent.render {|
        load = Examples.effectfulTabCounter
    |}
    ...
    "effectful-usecancellationtoken",
    DelayedComponent.render {|
        load = Examples.TokenCancellation.render
    |}
    ...
    "code-splitting-delayed",
    DelayedComponent.render {|
        load = Examples.codeSplittingDelayed
    |}
    "use-state-lazy",
    DelayedComponent.render {|
        load = Examples.useStateNormalVsLazy
    |}
    "use-deferred",
    DelayedComponent.render {|
        load = UseDeferredExamples.basicDeferred
    |}
    "deferred-form",
    DelayedComponent.render {|
        load = UseDeferredExamples.loginForm
    |}
    "use-deferred-v2",
    DelayedComponent.render {|
        load = UseDeferredExamples.basicDeferredV2
    |}
    "parallel-deferred",
    DelayedComponent.render {|
        load = UseDeferredExamples.parallelDeferred
    |}
    "use-elmish-basic", DelayedComponent.render {| load = UseElmishExamples.counter |}
    "use-elmish-combined",
    DelayedComponent.render {|
        load = UseElmishExamples.counterCombined
    |}
    "use-media-query",
    DelayedComponent.render {|
        load = UseMediaQueryExamples.useMediaQueryExample
    |}
    "use-responsive",
    DelayedComponent.render {|
        load = UseMediaQueryExamples.useResponsiveExample
    |}
    "use-responsive-custom",
    DelayedComponent.render {|
        load = UseMediaQueryExamples.useResponsiveCustomBreakpointsExample
    |}
    ...
    "delay-fallback",
    DelayedComponent.render {|
        load = DelayExamples.delayWithCustomFallback
    |}
    ...
    "delay-template",
    DelayedComponent.render {|
        load = DelayExamples.renderDelayTemplate
    |}
    "delay-suspense-template",
    DelayedComponent.render {|
        load = DelayExamples.renderDelaySuspenseTemplate
    |}
]
...
let githubPath (rawPath: string) =
    ...
    if parts.Length > 5 then
        sprintf "http://www.github.com/%s/%s" parts.[3] parts.[4]
    else
        rawPath
...
let renderMarkdown =
    React.functionComponent (fun (input: {| path: string; content: string |}) ->
        Html.div [
            ...
            prop.children [
                if input.path.StartsWith "https://raw.githubusercontent.com" then
                    Html.h2 [
                        Html.i [
                            prop.className [ FA.Fa; FA.FaGithub ]
                        ]
                        Html.anchor [
                            prop.style [
                                style.marginLeft 10
                ...
                Markdown.markdown [ markdown.escapeHtml false ]
            ]
        ]
    )
...
module MarkdownLoader =
        ...
        | StartLoading path ->
            let loadMarkdownAsync () = async {
                ...
                if statusCode = 200 then
                    return Loaded (Ok responseText)
                else
                    return Loaded (Error (statusCode, responseText))
            }

            Loading, Cmd.OfAsync.perform loadMarkdownAsync () id

        | Loaded (Ok content) -> State.LoadedMarkdown content, Cmd.none
    ...
    let load' =
        React.functionComponent (
            "LoadMarkdown",
            fun (props: {| path: string list |}) ->
                let (state, dispatch) =
                    React.useElmish (init props.path, update, [| box props.path |])
    ...
    React.functionComponent (fun
                                 (input:
                                     {|
                                         ...
                                         elems: (string list -> Fable.React.ReactElement) list
                                         dispatch: Msg -> unit
                                     |}) ->
        let collapsed =
        ...
        Html.li [
            Html.anchor [
                ...
                <| fun _ ->
                    ...
                    | true ->
                        input.dispatch
                        <| TabToggled (input.basePath |> List.rev |> List.tail |> List.rev)
...
// top level label
let menuLabel' =
    React.functionComponent (fun (input: {| content: string |}) ->
        Html.p [
            prop.className [
                Bulma.MenuLabel
                Bulma.IsUnselectable
            ]
...
let menuList' =
    React.functionComponent (fun
                                 (input:
                                     {|
                                         items: Fable.React.ReactElement list
                                     |}) ->
        Html.ul [
            prop.className Bulma.MenuList
...
let menuItem' =
    React.functionComponent (fun
        ...
        Html.li [
            Html.anchor [
                prop.className [
                    if input.currentPath = input.path then
                        Bulma.IsActive
                    if input.currentPath = input.path then
...
let menuLabel (content: string) = menuLabel' {| content = content |}

let menuList (items: Fable.React.ReactElement list) = menuList' {| items = items |}
...
let allItems =
    React.functionComponent (fun
        ...
        let nestedMenuList
            (name: string)
            (basePath: string list)
            (items: (string list -> Fable.React.ReactElement) list)
            =
            nestedMenuList' {|
                state = input.state
                name = name
        ...
        let subNestedMenuList
            (name: string)
            (basePath: string list)
            (items: (string list -> Fable.React.ReactElement) list)
            (addedBasePath: string list)
            =
            nestedMenuList' {|
                state = input.state
                name = name
            ...
            prop.children [
                menuList [
                    ...
                    nestedMenuList "Hooks" [ Urls.Hooks ] [
                        ...
                        nestedMenuItem "Feliz.Listener" [ Urls.Listeners ]
...
let loadOrSegment (origPath: string list) (basePath: string list) (path: string list) =
    if path |> List.isEmpty then
        Html.div [
            for segment in origPath -> Html.p segment
        ]
    else
        basePath @ path |> lazyView MarkdownLoader.load
...
let content =
    React.functionComponent (fun
        ...
        | PathPrefix [ Urls.UIFrameworks ] (Some res) ->
            ...
            | _ ->
                Html.div [
                    for segment in input.state.CurrentPath -> Html.p segment
                ]
        ...
        | PathPrefix [ Urls.Testing ] (Some res) ->
            ...
            | PathPrefix [ Urls.Utilities ] (Some res) ->
                match res with
                | [ Urls.RTL ] ->
                    lazyView MarkdownLoader.load [
                        "https://raw.githubusercontent.com/Shmew/Fable.Jester/master/src/Fable.ReactTestingLibrary/README.md"
                    ]
                | [ Urls.FastCheck ] ->
                    lazyView MarkdownLoader.load [
                        "https://raw.githubusercontent.com/Shmew/Fable.Jester/master/src/Fable.FastCheck/README.md"
                    ]
                | _ ->
                    Html.div [
                        for segment in input.state.CurrentPath -> Html.p segment
        ...
        | segments ->
            React.fragment [
                for segment in segments -> Html.p segment
            ]
    )
...
let render (state: State) dispatch =
    render' {| state = state; dispatch = dispatch |}
